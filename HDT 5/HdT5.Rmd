...
classifier <- naiveBayes(Group ~ LotArea,TotalBsmtSF,X1stFlrSF,X2ndFlrSF,WoodDeckSF,OpenPorchSF,
                EnclosedPorch,X3SsnPorch,ScreenPorch,PoolArea,MiscVal,data = trainingSet)
predicter <- predict(classifier,newdata = testingSet)
cm <- table(testingSet$Group,predicter)
# Matriz de confusion
confusionMatrix(cm)
```
### Bayes Ingenuo con Cross Validation
``` {r }
train_control <- trainControl(method = "cv", number = 10)

modelo = train(Group ~ LotArea+TotalBsmtSF+X1stFlrSF+X2ndFlrSF+WoodDeckSF+OpenPorchSF+
                EnclosedPorch+X3SsnPorch+ScreenPorch+PoolArea+MiscVal,data = trainingSet, method="nb", trControl = train_control)

prediction <- predict(modelo, newdata = testingSet)

cm2 <- table(testingSet$Group,prediction)
# Matriz de confusion
confusionMatrix(cm2)
```
