...
modelTwo <- svm(Group~.,data = datosTrain,type = "C-classification",kernel="linear", cost=1)
modelThree <- svm(Group~.,data = datosTrain,type = "C-classification",kernel="polynomial", cost=1, gamma=1)
```
...
### Plot 2
``` {r }
plot(modelTwo,datosTrain,EnclosedPorch~SalePrice)
prediccion <- predict(modelTwo,newdata=datosTest[,1:12])
confusionMatrix(datosTest$Group,prediccion)
```

### Plot 3
``` {r }
plot(modelThree,datosTrain,EnclosedPorch~SalePrice)
prediccion <- predict(modelThree,newdata=datosTest[,1:12])
confusionMatrix(datosTest$Group,prediccion)
```